"""
V1 朴素版 RAG：最基础的检索增强生成

- 固定大小切块（无重叠）
- 向量搜索 Top-K（无相似度过滤）
- 直接将检索结果塞进 prompt 生成回答
...
def simple_chunk(text, chunk_size=200):
    ...
    最朴素的切块方式：每 chunk_size 个字符切一刀。
    优点：简单粗暴，容易理解。
    缺点：可能把一句话从中间劈开，丢失上下文。
    """
    chunks = []
...
def rag_query(question):
    ...
    # 第一步：检索
    chunks, metadatas = search(question, top_k=3)
...
if __name__ == "__main__":
    ...
    # 测试几个问题
    rag_query("入职满5年有几天年假？")
    rag_query("出差住酒店的报销标准是多少？")
    rag_query("前端项目用什么构建工具？")
